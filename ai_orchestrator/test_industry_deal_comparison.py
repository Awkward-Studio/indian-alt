...
class IndustryDealComparisonTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.competitor_chunk = DocumentChunk.objects.create(
            ...
            content="Rival sells packaged foods to supermarkets in FY25.",
        )
        cls.peer_chunk = DocumentChunk.objects.create(
            deal=cls.peer, source_type="document", source_id=str(cls.peer_doc.id),
            content="Peer sells packaged foods through distributors in FY25.",
        )
        cls.unrelated_chunk = DocumentChunk.objects.create(
            ...
            content="Unrelated software revenue was INR 20 crore.",
        )
    ...
    def test_comparison_scopes_and_labels_ranked_source_chunks(self):
        ...
        self.assertEqual(result["chunks"], [self.competitor_chunk, self.peer_chunk])
        self.assertEqual(result["candidate_deal_count"], 2)
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            "same-sector peer candidate",
        )
        search = embedding.search_global_chunks.call_args
        self.assertTrue(search.kwargs["rerank"])
        self.assertNotIn(str(self.unrelated_doc.id), search.kwargs["source_ids"])
        self.assertNotIn(str(self.target_doc.id), search.kwargs["source_ids"])
    ...
    def test_industry_context_cites_peer_separately_from_target(self):
        ...
        self.assertIn("same-sector peer candidate", result["context"])
