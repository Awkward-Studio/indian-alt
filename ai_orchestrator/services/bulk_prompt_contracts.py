...
IC_REPORT_SECTION_PRESENTATION = {
    ...
    "Industry Overview": "Use a table for sourced market estimates, named competitors or demand drivers when their periods and units can be compared. Explain why the company can or cannot win share and which market assumption affects the forecast. Add a `### Our deal comparison` subsection using the separately retrieved internal database comparison blocks. The comparison retrieval searches indexed deal evidence semantically across the database, without an industry filter. Compare the target with source-backed competitors or peer candidates in a table with company, relationship status, product and customer overlap, comparable metric with period and unit when available, and source. Cite each factual row. A semantic peer candidate is not a confirmed direct competitor. Explain qualitative similarities even when numeric data is not comparable; name the mismatched business models, accounting bases or periods instead of inventing a metric. If no relevant comparison blocks were retrieved, say that no source-backed internal peer comparison is available and name the record or research needed. Never treat another deal's claims as facts about the target or as proof of total market share.",
