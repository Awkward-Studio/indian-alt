...
def process_single_document_async(
    ...
    try:
        ...
        artifact_source = (
            doc.evidence_json.get("source_metadata", {})
...
def _compact_document_artifact(doc: DealDocument) -> dict:
...
def synthesize_complete_deal_analysis(
    ...
    *,
    ...
    input_files = [
        {
            "file_id": doc.onedrive_id or str(doc.id),
            "file_name": doc.title,
            "document_id": str(doc.id),
            "transcription_status": doc.transcription_status,
...
def finalize_durable_vdr_indexing(self, audit_log_id: str):
    ...
    metadata = dict(audit.source_metadata or {})
    if metadata.get("queue_state") == "completed":
        return {
            "status": "completed",
            "analysis_id": metadata.get("field_synthesis_analysis_id"),
    ...
    document_ids = [
        ...
        if item.get("status") in {"completed", "cached"} and item.get("document_id")
    ]
    try:
        ...
        if (
            ...
            AIAuditLog.objects.filter(id=audit_log_id).update(
                ...
                source_metadata={
                    **metadata,
                    "queue_state": "field_synthesis_retry",
                    "workflow_stage": "field_synthesis_retry",
                    "field_synthesis_status": "retrying",
                },
            )
        ...
        AIAuditLog.objects.filter(id=audit_log_id).update(
            ...
            source_metadata={
                **metadata,
                "queue_state": "failed",
                "workflow_stage": "field_synthesis_failed",
                "field_synthesis_status": "failed",
            },
        )
        Deal.objects.filter(id=audit.source_id).update(
            processing_status="failed",
            processing_error=f"Documents were indexed, but deal fields could not be filled: {exc}",
        )
        failed_audit = AIAuditLog.objects.get(id=audit_log_id)
        broadcast_audit_log_update(failed_audit, event_type="terminal", done=True)
        vdr_queue.kick()
        raise
    ...
    try:
        ...
        if not errors and not remaining_documents:
            ...
            batch_document_ids = [
                ...
                if result.get("status") in {"success", "cached"} and result.get("document_id")
            ]
            log_worker_event(
                audit_log,
        ...
        if errors:
            ...
            audit_log.system_prompt = (
                f"Indexed {processed_count} documents and reused {cached_count} cached documents. "
                "Deal fields were refreshed from indexed evidence; the full report is waiting for user confirmation."
            )
        audit_log.source_metadata = {
            **(audit_log.source_metadata or {}),
        ...
        if (
            ...
            Deal.objects.filter(id=deal_id).update(
                ...
                processing_error=audit_log.error_message,
            )
...
def generate_vdr_analysis_async(
    ...
    try:
        ...
        log_worker_event(audit_log, f"VDR report generation failed: {exc}", status="FAILED", done=True)
        raise


@shared_task(bind=True, max_retries=3)
def prepare_linked_folder_vdr_async(
    self,
    deal_id: str,
    folder_id: str,
    drive_id: str,
    ...
    try:
        deal = Deal.objects.get(id=deal_id)
        VDRSyncService.sync_existing_analyses_to_folder(deal, user_email=user_email)
        FolderAnalysisService.persist_folder_tree(
            deal=deal,
            folder_id=folder_id,
            drive_id=drive_id,
            user_email=user_email,
...
def finalize_thread_analysis_async(self, results, deal_id: str | None, audit_log_id: str):
    ...
    if analysis and "error" not in analysis:
        ...
        # A valid JSON envelope is not proof that the report itself is complete.
        # Preserve a stable prefix, then generate and cache the missing tail one
        ...
        try:
            ...
            audit_log.error_message = error_msg
            audit_log.save(update_fields=["status", "is_success", "error_message"])
            log_worker_event(audit_log, error_msg, status="FAILED", done=True)
            return {"error": error_msg}
...
def _company_news_list(value) -> list:
    if isinstance(value, list):
        return [item for item in value if item]
    return []


def _company_news_item_text(item) -> str:
    if isinstance(item, str):
        return item.strip()
    if not isinstance(item, dict):
        return str(item).strip()

    title = item.get("title") or item.get("headline") or item.get("name") or item.get("topic") or "Finding"
    summary = item.get("summary") or item.get("detail") or item.get("description") or item.get("rationale") or ""
    ...
    if source or url:
        parts.append(f"Source: {' - '.join(str(part).strip() for part in [source, url] if part)}")
    return " | ".join(part for part in parts if part)


def _format_company_news_markdown(research: dict, *, company_name: str, generated_at) -> str:
    generated_label = generated_at.strftime("%Y-%m-%d %H:%M %Z") if hasattr(generated_at, "strftime") else str(generated_at)
    lines = [
        f"# Public Domain News Research - {company_name}",
        "",
        f"Generated: {generated_label}",
        "",
    ]

    executive_summary = research.get("executive_summary") or research.get("summary") or ""
    ...
    category_labels = [
        ...
        ("patents", "Patents / IP"),
        ("founders", "Founder / Promoter"),
        ("awards", "Awards / Recognition"),
        ("red_flags", "Red Flags"),
        ("green_flags", "Green Flags"),
        ("other", "Other Material News"),
    ]
    for key, label in category_labels:
        items = _company_news_list(research.get(key))
        if not items:
            continue
    ...
    sources = _company_news_list(research.get("sources"))
    if sources:
        ...
        for source in sources:
            if isinstance(source, dict):
                ...
                date = source.get("date") or source.get("published_at") or ""
                source_line = str(title)
                if date:
                    source_line += f" ({date})"
                if url:
                    source_line += f" - {url}"
...
def fetch_company_news_async_task(deal_id: str, instruction: str = "", existing_news: list[dict] | None = None) -> dict:
    ...
    try:
        ...
        doc = DealDocument.objects.create(
            ...
            transcription_status=TranscriptionStatus.COMPLETE,
            chunking_status=ChunkingStatus.NOT_CHUNKED,
            last_transcribed_at=generated_at,
        )

        artifact = {
            "document_name": title,
            "document_type": DocumentType.MEMO,
            "document_summary": research.get("executive_summary") or "",
            "claims": (
                _company_news_list(research.get("funding"))
                + _company_news_list(research.get("patents"))
                + _company_news_list(research.get("founders"))
                + _company_news_list(research.get("awards"))
                + _company_news_list(research.get("green_flags"))
                + _company_news_list(research.get("other"))
            ),
            "metrics": [],
            "tables_summary": [],
            "contacts_found": [],
        ...
        return {
            ...
            "indexed": bool(indexed or doc.is_indexed),
            "generated_at": generated_at.isoformat(),
        }
    except Exception as e:
