        ...
        'CONFIG': {
            "hosts": [{
                ...
                "socket_connect_timeout": config(
                    'CHANNEL_REDIS_CONNECT_TIMEOUT', default=5, cast=float,
                ),
                "socket_timeout": None,
                "health_check_interval": config(
                    'CHANNEL_REDIS_HEALTH_CHECK_INTERVAL', default=15, cast=int,
                ),
...
CELERY_TASK_ROUTES = {
    ...
    'microsoft.tasks.ingest_email_evidence': {'queue': 'email_priority'},
...
VDR_ARTIFACT_SPREADSHEET_SOURCE_TOKENS = config('VDR_ARTIFACT_SPREADSHEET_SOURCE_TOKENS', default=6000, cast=int)
VDR_ARTIFACT_SPREADSHEET_MAX_TOKENS = config('VDR_ARTIFACT_SPREADSHEET_MAX_TOKENS', default=16384, cast=int)
...
VDR_REPORT_SECTION_CANDIDATES = config('VDR_REPORT_SECTION_CANDIDATES', default=320, cast=int)
VDR_REPORT_SECTION_MAX_CHUNKS = config('VDR_REPORT_SECTION_MAX_CHUNKS', default=240, cast=int)
VDR_REPORT_SECTION_MIN_CHUNKS_PER_DOCUMENT = config('VDR_REPORT_SECTION_MIN_CHUNKS_PER_DOCUMENT', default=8, cast=int)
# Retrieved evidence uses about 36k tokens. Instructions and deal fields bring the
# complete section input near 40k, leaving 16k for output inside the 65k window.
...
VDR_DEPLOYMENT_HANDOFF_SECONDS = config('VDR_DEPLOYMENT_HANDOFF_SECONDS', default=90, cast=int)
...
VDR_INTERACTIVE_YIELD_RETRY_SECONDS = config(
    'VDR_INTERACTIVE_YIELD_RETRY_SECONDS', default=5, cast=int,
)
VDR_INTERACTIVE_UNACKED_STALE_SECONDS = config(
    'VDR_INTERACTIVE_UNACKED_STALE_SECONDS', default=3600, cast=int,
)
...
SEARCH_QUERY_PLANNER_ENABLED = config('SEARCH_QUERY_PLANNER_ENABLED', default=True, cast=bool)
SEARCH_QUERY_PLANNER_TIMEOUT = config('SEARCH_QUERY_PLANNER_TIMEOUT', default=45, cast=int)
...
CLAUDE_SEARCH_MODEL = config('CLAUDE_SEARCH_MODEL', default='claude-sonnet-4-6')
