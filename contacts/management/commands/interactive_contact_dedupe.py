...
from deals.services.entity_dedupe import (
    contact_candidate_groups,
    contact_retention_recommendations,
...
class Command(BaseCommand):
    help = "Interactively merge duplicate Contact rows while preserving deal and meeting relationships."

    def add_arguments(self, parser):
        parser.add_argument("--match", choices=["email", "name_bank", "name"], default="email")
    ...
    def handle(self, *args, **options):
        groups = contact_candidate_groups(match=options["match"])
        if options.get("key"):
        ...
        if not groups:
            self.stdout.write(self.style.SUCCESS("No duplicate contact candidate groups found."))
            return

        mode = "APPLY" if options["apply"] else "DRY-RUN"
        self.stdout.write(f"{mode}: reviewing {len(groups)} contact duplicate candidate group(s).")
        ...
        for group_index, group in enumerate(groups, 1):
            ...
            recommendations = contact_retention_recommendations(group.items)
            best_score = max((rec.score for rec in recommendations.values()), default=None)
            for index, contact in enumerate(group.items, 1):
                recommendation = recommendations.get(str(contact.id))
                recommended = ""
                if recommendation and recommendation.score == best_score:
                    recommended = "  [recommended canonical]"
                self.stdout.write(f"  {index}. {summarize_contact(contact)}{recommended}")
                self.stdout.write(f"     {format_recommendation(recommendation)}")

            recommended_indices = [
                index
                for index, contact in enumerate(group.items, 1)
                if recommendations.get(str(contact.id)) and recommendations[str(contact.id)].score == best_score
            ]
            auto_merge = False
            ...
            self.stdout.write(f"  Canonical: {summarize_contact(canonical)}")
            for duplicate in duplicates:
                self.stdout.write(f"  Merge:     {summarize_contact(duplicate)}")
            ...
            if not options["yes"] and not auto_merge:
                confirm = input("Apply this contact merge? This deletes duplicate contact rows. [y/N]: ").strip().lower()
                if confirm != "y":
                    continue
            merge_many_contacts(canonical, duplicates)
            self.stdout.write(self.style.SUCCESS("  Contact merge complete."))
